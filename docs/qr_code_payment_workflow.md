...
Người gửi (B) nhập MÃ PIN GIAO DỊCH và bấm "Xác nhận Thanh toán".
        │
        ▼
FE của B gọi: POST /api/v1/transactions/payment
        Header: Authorization: Bearer <JWT của B>
        Body:
        {
...
[API Gateway -> transaction-service]
  - Gateway forward sang POST /api/transactions/payment
  - TransactionType được set cứng = PAYMENT ở controller (FE không thể thay đổi)
  ...
  4. Lưu Database Transaction (Trạng thái PENDING, Type = PAYMENT).
  ...
  7. Publish event lên ActiveMQ (transactionType = PAYMENT) -> notification-service.
        │
        ▼
...
3. **Chống Bypass Payment:** Dù cho Hacker có tự đóng giả FE gọi trực tiếp vào API `POST /api/v1/transactions/payment` bằng tool (như Postman), tiền vẫn sẽ không lọt ra ngoài vì Hacker buộc phải biết mã `PIN` xác thực của người gửi. Ngoài ra, `TransactionType = PAYMENT` được set cứng ở backend, FE không thể tự ý đổi type.
