...
CREATE TABLE users (
    ...
    status               VARCHAR(20)  NOT NULL DEFAULT 'ACTIVE',          -- Trạng thái tài khoản: ACTIVE | LOCKED
    kyc_status           VARCHAR(20)  NOT NULL DEFAULT 'UNVERIFIED',      -- Trạng thái KYC: UNVERIFIED | PENDING | VERIFIED | REJECTED
...
-- -----------------------------------------------------------------------------
-- Bảng user_kyc: Thông tin xác minh danh tính người dùng
-- Chỉ bắt buộc khi user muốn sử dụng tính năng vay vốn hoặc các tính năng yêu cầu KYC
-- Khi admin duyệt/từ chối → đồng bộ users.kyc_status
-- -----------------------------------------------------------------------------
CREATE TABLE user_kyc (
    kyc_id            UUID         NOT NULL DEFAULT gen_random_uuid(), -- Khóa chính, UUID tự sinh
    user_id           UUID         NOT NULL,                           -- FK → users.user_id, mỗi user chỉ có 1 bản KYC

    -- Nhóm 1: Thông tin cá nhân (bắt buộc khi submit)
    full_name         VARCHAR(100) NOT NULL,                           -- Tên theo CCCD (có thể khác tên đăng ký)
    ...
    submitted_at      TIMESTAMP    NOT NULL,                           -- Thời điểm user nộp KYC
    verified_by       UUID,                                            -- UUID admin duyệt (null nếu chưa duyệt)
    verified_at       TIMESTAMP,                                       -- Thời điểm được duyệt (null nếu chưa duyệt)
    rejection_reason  VARCHAR(500),                                    -- Lý do từ chối (null nếu không bị từ chối)
    ...
    CONSTRAINT fk_user_kyc_user FOREIGN KEY (user_id) REFERENCES users (user_id),
    CONSTRAINT chk_user_kyc_status CHECK (status IN ('PENDING', 'VERIFIED', 'REJECTED'))
);

COMMENT ON TABLE user_kyc IS 'Thông tin KYC của người dùng, dùng để xác minh danh tính khi sử dụng tính năng vay vốn';
